...
from agents.ppo_agent import PPOAgent, PPOMemory, train_ppo_episode
...
if __name__ == '__main__':
    ...
    for ep in range(episodes):
        ...
        total_reward, sum_loss = train_ppo_episode(env, agent, memory, batch_size=batch_size)
    ...
    plt.subplot(1, 2, 2)
    plt.plot(all_losses, marker='s', color='orange')
    plt.xlabel('Episode')
    plt.ylabel('Loss')
    plt.title('PPO - Loss over Episodes')
    plt.grid(True)
    plt.tight_layout()
    plt.show()
