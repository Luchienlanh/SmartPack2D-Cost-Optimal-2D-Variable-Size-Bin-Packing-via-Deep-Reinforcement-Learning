...
from agents.pg_agent import PolicyNetwork, train_pg_episode
...
if __name__ == '__main__':
    ...
    checkpoint_path = os.path.join(root_dir, 'train_plc.pth')
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            ...
            if 'frame_net_state_dict' in checkpoint:
    ...
    for ep in range(episodes):
        ...
        print(f"Episode {ep + 1}/{episodes} [{file_name}], Reward={total_reward:.2f}, Cost={env.total_bin_cost:.1f}, Bins={env.opened_bins_count}")
    ...
    plt.title('Policy Gradient - Reward Over Episodes')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.grid(True)
    plt.show()
